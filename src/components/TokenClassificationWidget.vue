    ...
    >
      <div v-if="hasResponse">
        ...
        <span
          ...
          :key="index"
          :class="
            wordObject.class !== null
              ? wordObject.class.entity_group + ' all-entities'
              : ''
          "
        >
          <span>
          ...
          <span v-if="wordObject.class !== null" class="entity-group">{{
            wordObject.class.entity_group
          }}</span>
        </span>
      </div>
    </InferenceWidget>
  ...
  methods: {
    ...
    classifyWords: function(classes) {
      ...
      const reducer = (accumulatedWords, currentValue) => {
        const wordsFromLatest = this.sourceText.substring(
          latestIndex,
        ...
        const currentWordEndIndex =
          currentWordStartIndex + currentValue.word.length;

        const wordsUpToCurrent = this.sourceText.substring(
          latestIndex,
        ...
        const currentWordsInClass = this.sourceText.substring(
          currentWordStartIndex,
          currentWordEndIndex
        );
        // word with current entity
        const currentWordsObject = {
          word: currentWordsInClass,
          class: currentValue
        };

        latestIndex = currentWordEndIndex;

        return accumulatedWords.concat([
          wordsUpToCurrentObject,
          currentWordsObject
        ]);
      };
      ...
      if (latestIndex < this.sourceText.length - 1) {
        // include last words
        const finalWords = this.sourceText.substring(
          latestIndex,
          this.sourceText.length
        );
        return groupedWordsWithClasses.concat([
          { word: finalWords, class: null }
        ]);
      } else {
